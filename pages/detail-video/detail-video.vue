		...
		<scroll-view scroll-y="true" :style="'height:'+scrollH+'px;'">
			<view class="border-bottom border-light-secondary flex align-stretch bg-white" style="height: 80rpx;position: fixed;left: 0;right: 0;z-index: 100;">
				<view class="flex-1 flex align-center justify-center" v-for="(item,index) in tabBars" :key="index" @click="changeTab(index)">
					<text class="font py-1" :class="tabIndex === index ? 'text-main border-main':''" :style="tabIndex === index ? 'border-bottom: 5rpx solid;' : ''">{{item.name}}</text>
				</view>
			</view>
			...
			<view v-if="tabIndex === 0" class="animated fadeIn fast">
				...
				<view class="flex align-center justify-between text-muted px-3">
					...
					<view class="flex align-center justify-center rounded bg-main text-white ml-auto" hover-class="bg-main-hover"
					 @click="doFava">
				...
				<view class="py-3 border-top border-bottom border-light-secondary">
					...
					<scroll-view scroll-x="true" class="scroll-row mt-2">
						<view v-for="(item,index) in videos" :key="index" class="scroll-row-item rounded border ml-2 p-2 " :class="activeIndex === index ? 'text-main border-main' : 'text-muted border-light-secondary'"
						 style="width: 340rpx;" @click="changeVideo(item,index)">
			...
			<view v-else class="animated fadeIn fast px-3 py-1">
				...
				<view class="uni-comment-list" v-for="(item,index) in comments" :key='index'>
					<view class="uni-comment-face">
						<!-- 头像 -->
						<image :src="item.send_user.avatar || '/static/demo/6.jpg'" class="rounded-circle" style="width: 80rpx;height: 80rpx;"></image>
					</view>
					<view class="uni-comment-body">
						...
						<!-- 回复 -->
						<view class="flex flex-wrap px-2 bg-light rounded" v-for="(item2,index2) in item.comments" :key="index2">
				...
				<view class="position-fixed flex align-center justify-center bg-main rounded-circle" hover-class="bg-main-hover pulse"
				 style="width: 120rpx; height: 120rpx;right: 50rpx;bottom: 50rpx;" @click="openComment(0)">
				...
				<view style="height: 600rpx;">
					...
					<scroll-view scroll-y="true" style="height: 520rpx;">
						<view class="flex flex-wrap">
							<view v-for="(item,index) in videos" :key="index" style="width: 50%;box-sizing: border-box;" class="p-2">
								<view 
		...
		<f-popup ref="comment">
			<view class="position-absolute bottom-0 left-0 right-0 bg-white animated faster fadeIn" @click.stop="stop">
				<view class="flex align-center px-3" style="height: 100rpx;">
					<input type="text" v-model="content" class="bg-light rounded mr-3 px-2 flex-1" style="height: 60rpx;" :placeholder="reply_user.id ?'回复@'+reply_user.name:'发送评论'" />
					<view class="flex align-center justify-center rounded bg-main text-white py-1 px-2 ml-auto" hover-class="bg-main-hover"
					 @click="sendComment">
						<text class="font">发 送</text>
					</view>
				</view>
			</view>
		</f-popup>
