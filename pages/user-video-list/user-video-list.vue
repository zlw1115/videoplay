		...
		<block v-for="(item,index) in list" :key="index">
			...
			<view v-else class="bg-white">
				...
				<view style="height: 350rpx;" class="bg-light position-relative" hover-class="bg-hover-light">
					...
					<view v-if="!item.video" class="position-absolute left-0 right-0 top-0 bottom-0 flex flex-column align-center justify-center"
					 style="background-color:rgba(255,255,255,0.3) ;" @tap="upload(item)">
						<text class="iconfont iconjia" style="font-size: 50rpx;"></text>
						<text class="font text-muted">点击添加视频</text>
					</view>
				</view>

				<view v-if="item.video" class="flex align-center justify-center font border-bottom border-top" hover-class="bg-light"
				 style="height: 100rpx;" @click="upload(item)">
					点击切换视频
				</view>
				...
				<form-item label="描述">
					<textarea v-model="item.desc" placeholder="请填写视频描述" class="py-3" style="width:550rpx" />
					</form-item>
				<view class="flex" style="height: 100rpx;">
...
<script>
	...
	export default {
		...
		computed:{
			showButton(){
				return (this.list.filter(item=>item.isedit)).length ? false :true;
			}
		},
		onLoad(e) {
			if(!e.id){
