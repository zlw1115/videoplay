<template>
	<view>
		...
		<view class="flex align-center" style="height: 44rpx;" @click="back">
			<view class="flex align-center justify-center animated fast" style="width: 44px;height: 44px;" hover-class="text-main pulse">
		...
		<view class="px-4">
			<input type="text" class="uni-input bg-light mb-4 rounded" placeholder="手机号/用户名/邮箱" v-model="form.username" />
			<input type="text" class="uni-input bg-light mb-4 rounded" placeholder="请输入密码" v-model="form.password" />
			<input v-if="type==='reg'" type="text" class="uni-input bg-light mb-4 rounded" placeholder="请输入确认密码" v-model="form.repassword" />
		</view>

		<view class="px-4">
			<main-big-button @click="submit">{{type==='login'?'登录':'注册'}}</main-big-button>
